      ...
      new DPMainToolbar_Widget_ButtonGroup('action', [
        ...
        new DPMainToolbar_Widget_Button({
          ...
          'onClick': function(pEl) {
            ...
            this.toolbar.addWidget(new DPMainToolbar_Widget_Toolbar_Share());
            this.toolbar.toggle(true);
...
var DPMainToolbar_Widget_Button = new Class({
  ...
  render: function(pContainerEl) {
    ...
    this.mEl = new Element('a', {
        ...
        'events': {
          'click': function(pE) {
            this.fireEvent('click', [this.mEl, pE]);
...
var DPWorkBench_ResponsiveWidget = new Class({
  ...
  toggleVersion: function(pSize, pCheck) {
    ...
    if(!pCheck) {
      ...
      DSWindow.CreateConfirm(transtext('Are you sure you want to delete '+ pSize.text +'? Please note that you will not be able to undo.'), transtext('Delete '+ pSize.text +'? '), transtext('Yes'), transtext('No'), function() {
...
var DPSidebar_Widget_FloatingToolbar = new Class({
  ...
  adjustToSitemap: function(pSitemapHeight) {

    if(pSitemapHeight === undefined && document.getElement('.property-toolbar.page-inspector')) {
      pSitemapHeight = document.getElement('.property-toolbar.page-inspector').getSize().y
    }
    pSitemapHeight = pSitemapHeight || 0;
    var default_top = document.getElement("#trial-info") ? 75 : 50;
    var total = default_top + pSitemapHeight;

    this.mToolbarEl.setStyle('top',total);

    if(this.mToolbarEl.getElement('.sidebar-wrapper')){
      var inner_elements_height = this.mToolbarEl.getElements('.category-title, .search')
        .map(function(pEl) {
          return pEl.getHeight();
        })
        .sum()
      this.mToolbarEl.getElement('.sidebar-wrapper').setStyle('height', document.getElement('#sidebar').getSize().y -inner_elements_height - default_top - pSitemapHeight);
    }
  }
...
var dmsDPPreview_Sidebar = new Class({
  ...
  setActiveTab: function() {
    ...
    if(this.getConfig()['active_tab'] === undefined) {
